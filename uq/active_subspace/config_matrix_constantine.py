from uq.inversion.model_function import MatrixModel
import numpy as np

np.random.seed(123467)

bool_gradient = True  # gradient available
step_size = 10 ** -5  # for approximation of gradients
factor_N = 10  # local linear model
k = 5 + 1  # desired dimension of subspace +1
alpha = 2  # oversampling factor in [2,10]

bool_diagonal_matrix = False
bool_constantine = False  # use same matrices as constantine

M_boot = 100  # bootstrapping
...
print("config_matrix_constantine: Check Eigenvalues of constructed A")
print(np.max(eig_A - np.linalg.eigvals(A)))
